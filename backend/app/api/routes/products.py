...
@router.get('/available-products', response_model=List[Product])
async def get_products(skip: int=Query(0, ge=0, description='Number of records to skip for pagination'), limit: int=Query(100000, ge=1, le=100000, description='Max number of records to return'), status: Optional[str]=None, available_providers_id: Optional[int]=None, product_type: Optional[str]=None, db=Depends(get_db)):
    """
    ...
    """
    raise HTTPException(status_code=410, detail={'message': 'The available_products endpoint has been deprecated.', 'migration_info': 'Funds are now directly associated with portfolios instead of products. Please use the /funds and /available_portfolios endpoints instead.'})

@router.post('/available-products', response_model=Product)
...
@router.get('/available-products/{product_id}', response_model=Product)
...
@router.patch('/available-products/{product_id}', response_model=Product)
...
@router.delete('/available-products/{product_id}', response_model=dict)
...
@router.get('/product-value-irr-summary', response_model=List[dict])
