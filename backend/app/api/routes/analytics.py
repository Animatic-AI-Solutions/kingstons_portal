...
_company_irr_cache = {'value': None, 'timestamp': None, 'cache_duration': 86400}
...
@router.get('/analytics/fund-distribution')
async def get_fund_distribution(limit: int=Query(100000, ge=1, le=100000, description='Maximum number of funds to return'), db=Depends(get_db)):
    """
        ...
        for fund_record in funds_result:
            ...
            for pf in portfolio_funds:
                ...
                if pf_id in valuations_by_pf_id and valuations_by_pf_id[pf_id] is not None:
                    ...
                    valuations_used += 1
                elif pf['amount_invested'] is not None:
            ...
            if total_value > 0:
                fund_data.append({'id': fund['id'], 'name': fund['fund_name'], 'amount': total_value, 'category': 'fund', 'valuation_source': 'latest_valuation' if fund_has_valuations else 'amount_invested'})
...
@router.get('/analytics/provider-distribution')
async def get_provider_distribution(limit: int=Query(100000, ge=1, le=100000, description='Maximum number of providers to return'), db=Depends(get_db)):
    """
                ...
                if portfolio_id:
                    ...
                    for pf in portfolio_funds:
                        ...
                        if pf_id in valuations_by_pf_id and valuations_by_pf_id[pf_id] is not None:
                            ...
                            logger.debug(f"No valuation found for portfolio_fund {pf['id']}, using amount_invested: {pf['amount_invested']}")
            if total_value > 0:
                provider_data.append({'id': str(provider['id']), 'name': provider['name'], 'amount': total_value, 'valuation_source': 'latest_valuation' if provider_has_valuations else 'amount_invested'})
...
async def get_dashboard_stats(db=Depends(get_db)):
    ...
    """
    try:
        stats = {'totalFUM': 0, 'companyIRR': 0, 'totalAccounts': 0, 'totalClients': 0, 'totalActiveHoldings': 0}
        try:
...
@router.get('/analytics/performance-data')
async def get_performance_data(date_range: Literal['all-time', 'ytd', '12m', '3y', '5y']='all-time', entity_type: Literal['overview', 'clients', 'products', 'portfolios', 'funds', 'products', 'providers']='overview', sort_order: Literal['highest', 'lowest']='highest', limit: int=Query(100000, ge=1, le=100000), db=Depends(get_db)):
    try:
        response = {'companyFUM': 0, 'companyIRR': 0, 'performanceData': []}
        ...
        if entity_type == 'funds':
            ...
            if funds_result:
                ...
                if total_weight > 0:
                    response['performanceData'].append({'id': fund['id'], 'name': fund['fund_name'], 'type': 'fund', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': None})
            response['performanceData'].sort(key=lambda x: x['irr'], reverse=sort_order == 'highest')
            ...
            if portfolios_result:
                ...
                for pf in portfolio_funds:
                    ...
                    if pf_id in irr_by_pf_id and amount_invested:
                        ...
                        total_weight += amount_invested
                if total_weight > 0:
                    response['performanceData'].append({'id': portfolio['id'], 'name': portfolio['portfolio_name'], 'type': 'portfolio', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': portfolio['start_date']})
            response['performanceData'].sort(key=lambda x: x['irr'], reverse=sort_order == 'highest')
            response['performanceData'] = response['performanceData'][:limit]
            ...
            if products_result:
                ...
                for product_record in products_result:
                    ...
                    if total_weight > 0:
                        ...
                        response['performanceData'].append({'id': product['id'], 'name': product['product_name'] or f"{client_name}'s product", 'type': 'product', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': product['start_date']})
            response['performanceData'].sort(key=lambda x: x['irr'], reverse=sort_order == 'highest')
            response['performanceData'] = response['performanceData'][:limit]
            ...
            if clients_result:
                ...
                if total_weight > 0:
                    ...
                    response['performanceData'].append({'id': client['id'], 'name': client_name, 'type': 'client', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': earliest_start_date, 'advisor': client['advisor']})
            response['performanceData'].sort(key=lambda x: x['irr'], reverse=sort_order == 'highest')
            response['performanceData'] = response['performanceData'][:limit]
            ...
            for fund_record in funds_result:
                ...
                for pf in portfolio_funds:
                    ...
                    if pf_id in irr_by_pf_id and amount_invested:
                        ...
                        total_weight += amount_invested
                if total_weight > 0:
                    all_performers.append({'id': fund['id'], 'name': fund['fund_name'], 'type': 'fund', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': None})
            for portfolio_record in portfolios_result:
                portfolio = dict(portfolio_record)
                portfolio_id = portfolio['id']
                total_fum = 0
                ...
                for pf in portfolio_funds:
                    ...
                    if pf_id in irr_by_pf_id and amount_invested:
                        ...
                        total_weight += amount_invested
                if total_weight > 0:
                    all_performers.append({'id': portfolio['id'], 'name': portfolio['portfolio_name'], 'type': 'portfolio', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': portfolio['start_date']})
            for product_record in products_result:
                product = dict(product_record)
                portfolio_id = product['portfolio_id']
                if portfolio_id:
                    ...
                    if total_weight > 0:
                        ...
                        all_performers.append({'id': product['id'], 'name': product['product_name'] or f"{client_name}'s product", 'type': 'product', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': product['start_date']})
            for client_record in clients_result:
                client = dict(client_record)
                client_id = client['id']
                total_fum = 0
                        ...
                        for pf in portfolio_funds:
                            ...
                            if pf_id in irr_by_pf_id and amount_invested:
                                ...
                                if not earliest_start_date or (product['start_date'] and product['start_date'] < earliest_start_date):
                                    earliest_start_date = product['start_date']
                if total_weight > 0:
                    all_performers.append({'id': client['id'], 'name': client['name'], 'type': 'client', 'irr': weighted_irr / total_weight, 'fum': total_fum, 'startDate': earliest_start_date, 'advisor': client['advisor']})
            all_performers.sort(key=lambda x: x['irr'], reverse=sort_order == 'highest')
...
@router.get('/analytics/product-client-counts')
async def get_product_client_counts(db=Depends(get_db)):
    ...
    try:
        ...
        if not products_result:
            return {'relationships': [], 'products': [], 'data': []}
        ...
        for relationship in relationships:
            ...
            data.append({'relationship': relationship, 'counts': counts})
        return {'relationships': relationships, 'products': product_types, 'data': data}
    except Exception as e:
...
async def get_portfolio_performance(portfolio_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if not funds_result:
            return {'irr': 0.0, 'total_investment': 0.0, 'current_value': 0.0, 'fund_performance': []}
        fund_ids = [dict(fund)['id'] for fund in funds_result]
        ...
        for fund_record in funds_result:
            ...
            try:
                ...
                annualized_irr = (1 + irr) ** (365 / 365) - 1
            except:
                annualized_irr = 0.0
            fund_performance.append({'fund_id': fund['id'], 'fund_name': fund.get('fund_name', 'Unknown Fund'), 'investment': investment, 'current_value': fund_current_value, 'irr': annualized_irr})
        try:
            import numpy_financial as npf
            cash_flows = [-total_investment, current_value]
            irr = npf.irr(cash_flows)
            portfolio_irr = (1 + irr) ** (365 / 365) - 1
        except:
            portfolio_irr = 0.0
        return {'irr': portfolio_irr, 'total_investment': total_investment, 'current_value': current_value, 'fund_performance': fund_performance}
    except Exception as e:
...
async def calculate_company_irr(db):
    ...
    if _company_irr_cache['value'] is not None and _company_irr_cache['timestamp'] is not None and (current_time - _company_irr_cache['timestamp'] < _company_irr_cache['cache_duration']):
    ...
    try:
        ...
        irr_result = await calculate_multiple_portfolio_funds_irr(portfolio_fund_ids=all_portfolio_fund_ids, irr_date=None, db=db)
...
async def get_company_irr_endpoint(db=Depends(get_db)):
    ...
    try:
        ...
        return {'company_irr': company_irr, 'client_count': client_count, 'total_products': total_products, 'total_portfolio_funds': total_portfolio_funds, 'total_investment': total_investment}
    except Exception as e:
...
async def calculate_client_irr(client_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            return {'client_id': client_id, 'irr': client_irr, 'product_count': product_count, 'portfolio_fund_count': len(all_portfolio_fund_ids), 'calculation_method': 'standardized_multiple_funds_irr'}
        except Exception as irr_error:
            logger.error(f'Error calculating standardized IRR: {str(irr_error)}')
            return {'client_id': client_id, 'irr': 0, 'product_count': product_count, 'portfolio_fund_count': len(all_portfolio_fund_ids), 'error': f'IRR calculation failed: {str(irr_error)}'}
    except Exception as e:
...
async def calculate_product_irr(product_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if not product_result or not dict(product_result)['portfolio_id']:
            ...
            return {'product_id': product_id, 'irr': 0, 'portfolio_fund_count': 0, 'total_invested': 0, 'date': None}
        ...
        from app.api.routes.portfolio_funds import calculate_multiple_portfolio_funds_irr
        try:
            irr_result = await calculate_multiple_portfolio_funds_irr(portfolio_fund_ids=portfolio_fund_ids, irr_date=None, db=db)
            product_irr = irr_result.get('irr_percentage', 0)
            if product_irr == 0:
                product_irr = '-'
            return {'product_id': product_id, 'irr': product_irr, 'portfolio_fund_count': len(portfolio_fund_ids), 'total_valuation': irr_result.get('total_valuation', 0), 'date': irr_result.get('calculation_date'), 'calculation_method': 'standardized_multiple_funds_irr'}
        except Exception as irr_error:
            logger.error(f'Error calculating standardized IRR for product {product_id}: {str(irr_error)}')
            return {'product_id': product_id, 'irr': '-', 'portfolio_fund_count': len(portfolio_fund_ids), 'total_invested': 0, 'date': None, 'error': f'IRR calculation failed: {str(irr_error)}'}
    except Exception as e:
...
@router.get('/analytics/client-risks')
        ...
        for client_record in clients_result:
            try:
                ...
                if total_client_investment > 0:
                    ...
                    client_name = client['name']
                    client_risks.append({'client_id': client['id'], 'client_name': client_name, 'risk_score': round(client_risk, 2), 'total_investment': total_client_investment})
            except Exception as e:
...
async def get_portfolio_irr(portfolio_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if latest_irr_result and dict(latest_irr_result)['irr_result'] is not None:
            ...
            try:
                irr_calculation_result = await calculate_multiple_portfolio_funds_irr(portfolio_fund_ids=fund_ids, irr_date=None, db=db)
...
@router.get('/analytics/portfolio-template-distribution')
async def get_portfolio_template_distribution(limit: int=Query(100000, ge=1, le=100000, description='Maximum number of portfolio templates to return'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        for generation_record in generations_result:
            ...
            if total_value > 0:
                template_data.append({'id': str(generation['id']), 'name': generation['generation_name'], 'amount': total_value, 'valuation_source': 'latest_valuation' if template_has_valuations else 'amount_invested'})
        bespoke_total = 0
            ...
            for pf in portfolio_funds:
                ...
                if pf_id in valuations_by_pf_id:
                    ...
                    if amount_invested is not None:
                        ...
                        logger.debug(f'No valuation found for portfolio_fund {pf_id}, using amount_invested: {amount_invested}')
        if bespoke_total > 0:
            template_data.insert(0, {'id': 'bespoke', 'name': 'Bespoke Portfolios', 'amount': bespoke_total, 'valuation_source': 'latest_valuation' if bespoke_has_valuations else 'amount_invested'})
...
@router.get('/analytics/dashboard-all')
async def get_dashboard_all_data(fund_limit: int=Query(100000, ge=1, le=100000, description='Maximum number of funds to return'), provider_limit: int=Query(100000, ge=1, le=100000, description='Maximum number of providers to return'), template_limit: int=Query(100000, ge=1, le=100000, description='Maximum number of templates to return'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        templates_list = [{'id': str(item[0]), 'name': item[1], 'amount': item[2]} for item in template_items[:template_limit]]
        ...
        response = {'metrics': {'totalFUM': final_fum, 'companyIRR': company_irr, 'totalClients': client_count, 'totalAccounts': total_products, 'totalActiveHoldings': len(portfolio_funds_result) if portfolio_funds_result else 0}, 'funds': funds_list, 'providers': providers_list, 'templates': templates_list, 'performance': {'optimization_stats': {'total_db_queries': 6, 'total_portfolio_funds': len(portfolio_funds_result) if portfolio_funds_result else 0, 'total_valuations': len(all_valuations_result) if all_valuations_result else 0, 'valuations_used': len(valuations_lookup), 'fum_source': 'valuations' if total_fum > 0 else 'investments'}}}
        return response
    except Exception as e:
        logger.error(f'Error fetching optimized dashboard data: {str(e)}')
        raise HTTPException(status_code=500, detail=f'Error: {str(e)}')

@router.get('/analytics/risk-differences')
async def get_products_risk_differences(limit: int=Query(100000, ge=1, le=100000, description='Number of products to return'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        for product_record in products_with_clients_result:
            ...
            for pf in portfolio_funds:
                ...
                fund_data[pf_id] = {'risk_factor': fund_info['risk_factor'], 'fund_name': fund_info['fund_name'], 'target_weighting': float(pf['target_weighting'] or 0), 'amount_invested': float(pf['amount_invested'] or 0), 'latest_valuation': float(valuations_by_pf_id.get(pf_id, 0)) if pf_id in valuations_by_pf_id else None}
            if not fund_data:
                continue
            target_risk = None
            total_target_weight = sum((fund['target_weighting'] for fund in fund_data.values()))
            if total_target_weight > 0:
                weighted_target_risk = sum((fund['risk_factor'] * fund['target_weighting'] for fund in fund_data.values()))
            ...
            if total_valuation > 0 and funds_with_valuations:
                weighted_actual_risk = sum((fund['risk_factor'] * fund['latest_valuation'] for fund in funds_with_valuations.values()))
            ...
            if target_risk is not None and actual_risk is not None:
                ...
                risk_differences.append({'product_id': product_id, 'product_name': product['product_name'], 'client_name': client_name, 'target_risk': round(target_risk, 2), 'actual_risk': round(actual_risk, 2), 'risk_difference': round(risk_difference, 2), 'fund_count': len(fund_data), 'funds_with_valuations': len(funds_with_valuations)})
...
async def reset_company_irr_cache():
    ...
    return {'success': True, 'message': 'Company IRR cache has been reset', 'previous_value': old_value}

@router.get('/analytics/dashboard-fast')
async def get_ultra_fast_dashboard(fund_limit: int=Query(100000, ge=1, le=100000), provider_limit: int=Query(100000, ge=1, le=100000), template_limit: int=Query(100000, ge=1, le=100000), db=Depends(get_db)):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail='Analytics views not deployed. Please run deploy_analytics_views.ps1 script to enable ultra-fast analytics.')
        ...
        if funds_result and total_fum > 0:
            for fund_record in funds_result:
                ...
                funds.append({'id': fund['id'], 'name': fund['name'], 'amount': amount, 'percentage': round(amount / total_fum * 100, 1)})
        providers = []
        if providers_result and total_fum > 0:
            for provider_record in providers_result:
                provider = dict(provider_record)
                amount = float(provider['amount'] or 0)
                providers.append({'id': provider['id'], 'name': provider['name'], 'amount': amount, 'percentage': round(amount / total_fum * 100, 1)})
        templates = []
        if templates_result:
            for template_record in templates_result:
                template = dict(template_record)
                templates.append({'id': template['id'], 'name': template['generation_name'], 'amount': 0, 'percentage': 0})
        try:
            ...
            if revenue_result:
                ...
                revenue_data = {'total_annual_revenue': 0, 'active_products': 0, 'revenue_generating_products': 0, 'avg_revenue_per_product': 0, 'active_providers': 0}
        ...
        response = {'optimized': True, 'phase': 'ultra-fast', 'calculation_time': round(calculation_time, 2), 'metrics': {'totalFUM': float(summary.get('total_fum', 0)), 'companyIRR': float(summary.get('company_irr', 0)), 'totalClients': int(summary.get('total_clients', 0)), 'totalAccounts': int(summary.get('total_accounts', 0)), 'totalActiveHoldings': int(summary.get('total_funds_managed', 0))}, 'distributions': {'funds': funds, 'providers': providers, 'templates': templates}, 'performance': {'topPerformers': top_performers, 'clientRisks': client_risks}, 'revenue': revenue_data, 'cache_info': {'last_irr_calculation': summary.get('last_irr_calculation'), 'data_source': 'pre_computed_views'}}
        return response
    except Exception as e:
        end_time = time.time()
        calculation_time = end_time - start_time
...
async def refresh_company_irr_background(db=Depends(get_db)):
    ...
    try:
        ...
        async def background_irr_calculation():
            ...
            return {'success': True, 'company_irr': company_irr, 'calculation_time': round(calculation_time, 2), 'cache_updated': True, 'background': True}
        result = await background_irr_calculation()
        return result
    except Exception as e:
        logger.error(f'❌ Background company IRR calculation failed: {e}')
        return {'success': False, 'error': str(e), 'background': True}
...
async def get_irr_calculation_status():
    ...
    try:
        ...
        return {'cache_exists': _company_irr_cache['value'] is not None, 'cache_fresh': cache_fresh, 'cache_age_seconds': cache_age, 'cache_age_hours': round(cache_age / 3600, 1) if cache_age else None, 'cached_irr': _company_irr_cache['value'], 'cache_duration': _company_irr_cache['cache_duration'], 'last_calculation': _company_irr_cache['timestamp']}
    except Exception as e:
