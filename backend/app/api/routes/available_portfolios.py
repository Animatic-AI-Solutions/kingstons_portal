...
class PortfolioTemplateUpdate(BaseModel):
    name: Optional[str] = None
    created_at: Optional[datetime] = None
router = APIRouter(prefix='/available-portfolios')
        ...
        for idx, portfolio_record in enumerate(response):
            try:
                ...
                try:
                    ...
                    generations_response = await db.fetchrow("SELECT id, generation_name, status FROM template_portfolio_generations WHERE available_portfolio_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1", portfolio_id)
                    if generations_response:
                        ...
                        weighted_risk_response = await db.fetchrow('SELECT weighted_risk_factor FROM template_generation_weighted_risk WHERE id = $1', generation_id)
                ...
                portfolio_model = AvailablePortfolio(id=portfolio_id, created_at=str(portfolio['created_at']), name=portfolio.get('name'), weighted_risk=weighted_risk, generation_id=generation_id, generation_name=generation_name)
...
async def get_portfolio_templates_with_counts(db=Depends(get_db)):
    ...
    try:
        ...
        if not templates_response:
            return []
        generations_response = await db.fetch("SELECT id, available_portfolio_id, generation_name, status, created_at FROM template_portfolio_generations WHERE status = 'active'")
        if generations_response:
            generation_ids = [g['id'] for g in generations_response]
            weighted_risks_response = await db.fetch('SELECT id, weighted_risk_factor FROM template_generation_weighted_risk WHERE id = ANY($1::int[])', generation_ids)
            weighted_risks_map = {wr['id']: float(wr['weighted_risk_factor']) for wr in weighted_risks_response or [] if wr.get('weighted_risk_factor')}
        else:
            weighted_risks_map = {}
        if generations_response:
            portfolio_counts_response = await db.fetch('SELECT template_generation_id FROM portfolios WHERE template_generation_id = ANY($1::int[])', generation_ids)
        ...
        for template in templates_response:
            ...
            total_portfolio_count = sum((portfolio_counts_map.get(gen['id'], 0) for gen in template_generations))
            template_data = {'id': template_id, 'name': template.get('name'), 'created_at': template.get('created_at'), 'portfolioCount': total_portfolio_count, 'weighted_risk': weighted_risks_map.get(latest_generation['id']) if latest_generation else None, 'generation_id': latest_generation['id'] if latest_generation else None, 'generation_name': latest_generation['generation_name'] if latest_generation else None, 'status': template.get('status', 'active')}
    ...
    try:
        ...
        if generation_id:
            try:
                ...
                generation_response = await db.fetchrow('SELECT * FROM template_portfolio_generations WHERE id = $1', generation_id)
            ...
            try:
                ...
                generation_response = await db.fetchrow("SELECT * FROM template_portfolio_generations WHERE available_portfolio_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1", portfolio_id)
        ...
        try:
            if generation:
                ...
                portfolio_funds_response = await db.fetch('SELECT * FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation['id'])
            else:
        ...
        if portfolio_funds_response:
            for idx, portfolio_fund in enumerate(portfolio_funds_response):
                ...
                try:
                    ...
                    fund_response = await db.fetchrow('SELECT * FROM available_funds WHERE id = $1', portfolio_fund['fund_id'])
                    logger.info(f'Fund details response: {fund_response}')
                    fund_detail = {'id': portfolio_fund['id'], 'created_at': portfolio_fund['created_at'], 'portfolio_id': portfolio_id, 'target_weighting': float(portfolio_fund['target_weighting']) if portfolio_fund['target_weighting'] else 0.0, 'fund_id': portfolio_fund['fund_id'], 'available_funds': dict(fund_response) if fund_response else {}}
        ...
        if generation:
            try:
                ...
                weighted_risk_response = await db.fetchrow('SELECT weighted_risk_factor FROM template_generation_weighted_risk WHERE id = $1', generation['id'])
        ...
        response = {'id': portfolio['id'], 'created_at': portfolio['created_at'], 'name': portfolio['name'], 'funds': funds, 'weighted_risk': weighted_risk}
        if generation:
...
async def create_available_portfolio(portfolio_data: PortfolioCreate, db=Depends(get_db)):
    ...
    try:
        new_portfolio = {'name': portfolio_data.name}
        portfolio_response = await db.fetchrow('INSERT INTO available_portfolios (name) VALUES ($1) RETURNING *', new_portfolio['name'])
        if not portfolio_response:
        ...
        generation_data = {'available_portfolio_id': new_portfolio_id, 'generation_name': portfolio_data.generation_name or f'Initial generation of {portfolio_data.name}', 'description': portfolio_data.description, 'status': 'active'}
        generation_response = await db.fetchrow('INSERT INTO template_portfolio_generations (available_portfolio_id, generation_name, description, status) VALUES ($1, $2, $3, $4) RETURNING *', generation_data['available_portfolio_id'], generation_data['generation_name'], generation_data['description'], generation_data['status'])
        if not generation_response:
            ...
            try:
                async with db.transaction():
                    for fund in portfolio_data.funds:
                        fund_exists = await db.fetchrow('SELECT id FROM available_funds WHERE id = $1', fund.fund_id)
                        if not fund_exists:
                            logger.warning(f'Fund {fund.fund_id} does not exist, skipping')
                            continue
                        fund_response = await db.fetchrow('INSERT INTO available_portfolio_funds (template_portfolio_generation_id, fund_id, target_weighting) VALUES ($1, $2, $3) RETURNING *', new_generation_id, fund.fund_id, fund.target_weighting)
...
async def create_portfolio_from_template(data: PortfolioFromTemplate, db=Depends(get_db)):
    ...
    template_response = await db.fetchrow('SELECT * FROM available_portfolios WHERE id = $1', data.template_id)
    if not template_response:
        raise HTTPException(status_code=404, detail='Portfolio template not found')
    latest_generation_response = await db.fetchrow("SELECT * FROM template_portfolio_generations WHERE available_portfolio_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1", data.template_id)
    ...
    portfolio_response = await db.fetchrow('INSERT INTO portfolios (portfolio_name, status, start_date, template_generation_id) VALUES ($1, $2, $3, $4) RETURNING *', data.portfolio_name, 'active', str(date.today()), data.template_id)
    if not portfolio_response:
        raise HTTPException(status_code=500, detail='Failed to create portfolio')
    new_portfolio = dict(portfolio_response)
    template_funds_response = await db.fetch('SELECT * FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', latest_generation['id'])
    if not template_funds_response:
        return new_portfolio
    for template_fund in template_funds_response:
        template_fund = dict(template_fund)
        portfolio_fund_response = await db.fetchrow('INSERT INTO portfolio_funds (portfolio_id, available_funds_id, target_weighting, start_date) VALUES ($1, $2, $3, $4) RETURNING *', new_portfolio['id'], template_fund['fund_id'], template_fund['target_weighting'], str(date.today()))
...
async def delete_available_portfolio(portfolio_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        generations_result = await db.fetch('SELECT id, generation_name FROM template_portfolio_generations WHERE available_portfolio_id = $1', portfolio_id)
        ...
        if generation_ids:
            client_products_result = await db.fetch('SELECT id, product_name, template_generation_id FROM client_products WHERE template_generation_id = ANY($1::int[])', generation_ids)
            ...
            portfolios_result = await db.fetch('SELECT id, portfolio_name, template_generation_id FROM portfolios WHERE template_generation_id = ANY($1::int[])', generation_ids)
        ...
        return {'message': f"Portfolio template '{template_name}' deleted successfully", 'details': {'template_deleted': 1, 'generations_deleted': generations_count, 'funds_deleted': funds_count, 'client_products_deleted': client_products_deleted, 'portfolios_deleted': portfolios_deleted, 'template_name': template_name}}
        ...
        if 'foreign key' in error_msg or 'constraint' in error_msg:
            raise HTTPException(status_code=409, detail='Cannot delete template: There are still references to this template in the database. This may indicate a data integrity issue - please contact support.')
        else:
...
@router.get('/available-portfolio-funds', response_model=List[dict])
async def get_available_portfolio_funds(template_portfolio_generation_id: Optional[int]=None, fund_id: Optional[int]=None, db=Depends(get_db)):
    """
...
@router.get('/available-portfolio-funds/by-fund/{fund_id}', response_model=List[dict])
async def get_portfolio_funds_by_fund_id(fund_id: int, db=Depends(get_db)):
    """
    ...
    try:
        ...
        result = await db.fetch('SELECT * FROM available_portfolio_funds WHERE fund_id = $1', fund_id)
        logger.info(f'Query result: {result}')
...
async def get_portfolio_generations(portfolio_id: int, db=Depends(get_db)):
    ...
    try:
        template_response = await db.fetchrow('SELECT * FROM available_portfolios WHERE id = $1', portfolio_id)
        if not template_response:
            raise HTTPException(status_code=404, detail='Portfolio template not found')
        generations_response = await db.fetch('SELECT * FROM template_portfolio_generations WHERE available_portfolio_id = $1 ORDER BY created_at DESC', portfolio_id)
...
async def create_portfolio_generation(portfolio_id: int, generation_data: GenerationCreate, db=Depends(get_db)):
    ...
    try:
        ...
        if generation_data.copy_from_generation_id:
            source_generation_response = await db.fetchrow('SELECT * FROM template_portfolio_generations WHERE id = $1', generation_data.copy_from_generation_id)
            if not source_generation_response:
                ...
                source_funds_response = await db.fetch('SELECT * FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_data.copy_from_generation_id)
                    ...
                    try:
                        async with db.transaction():
                            for source_fund in source_funds_response:
                                ...
                                new_fund = {'template_portfolio_generation_id': new_generation_id, 'fund_id': source_fund['fund_id'], 'target_weighting': source_fund['target_weighting']}
                                await db.execute('INSERT INTO available_portfolio_funds (template_portfolio_generation_id, fund_id, target_weighting) VALUES ($1, $2, $3)', new_fund['template_portfolio_generation_id'], new_fund['fund_id'], new_fund['target_weighting'])
...
async def update_generation(portfolio_id: int, generation_id: int, update_data: Union[GenerationStatusUpdate, GenerationUpdate], db=Depends(get_db)):
    """Update a generation's details, including status and/or funds"""
    try:
        generation_response = await db.fetchrow('SELECT * FROM template_portfolio_generations WHERE id = $1 AND available_portfolio_id = $2', generation_id, portfolio_id)
        if not generation_response:
            raise HTTPException(status_code=404, detail=f'Generation with ID {generation_id} not found for portfolio {portfolio_id}')
        if hasattr(update_data, 'status') and update_data.status:
            valid_statuses = ['active', 'draft', 'archived']
            if update_data.status not in valid_statuses:
                raise HTTPException(status_code=422, detail=f"Invalid status value. Must be one of: {', '.join(valid_statuses)}")
            if update_data.status == 'active':
                other_active_response = await db.fetch("SELECT id FROM template_portfolio_generations WHERE available_portfolio_id = $1 AND status = 'active' AND id != $2", portfolio_id, generation_id)
                if other_active_response:
                    for other_generation in other_active_response:
                        await db.execute("UPDATE template_portfolio_generations SET status = 'archived' WHERE id = $1", other_generation['id'])
        ...
        if hasattr(update_data, 'funds') and update_data.funds is not None:
            try:
                ...
                existing_funds = await db.fetch('SELECT id, fund_id, target_weighting FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_id)
                logger.info(f'Found {(len(existing_funds) if existing_funds else 0)} existing funds for generation {generation_id}')
                async with db.transaction():
                    await db.execute('DELETE FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_id)
                    ...
                    for i, fund in enumerate(valid_funds):
                        try:
                            fund_response = await db.fetchrow('INSERT INTO available_portfolio_funds (template_portfolio_generation_id, fund_id, target_weighting) VALUES ($1, $2, $3) RETURNING id', generation_id, fund.fund_id, fund.target_weighting)
                            if fund_response:
                                inserted_count += 1
                            ...
                            if 'duplicate key' in error_msg and 'primary key' in error_msg:
                                ...
                                try:
                                    ...
                                    await db.execute(f'ALTER SEQUENCE available_portfolio_funds_id_seq RESTART WITH {next_val}')
        ...
        updated_generation = await db.fetchrow('SELECT * FROM template_portfolio_generations WHERE id = $1', generation_id)
...
async def delete_generation(portfolio_id: int, generation_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if not generation_response:
            raise HTTPException(status_code=404, detail=f'Generation with ID {generation_id} not found for portfolio {portfolio_id}')
        generation = dict(generation_response)
        portfolios_using_generation = await db.fetch('SELECT id FROM portfolios WHERE template_generation_id = $1', generation_id)
        if portfolios_using_generation and len(portfolios_using_generation) > 0:
            portfolio_count = len(portfolios_using_generation)
            raise HTTPException(status_code=400, detail=f"Cannot delete generation '{generation['generation_name']}' - {portfolio_count} portfolio(s) are using this generation")
        funds_response = await db.fetch('SELECT id FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_id)
        funds_count = len(funds_response) if funds_response else 0
        delete_result = await db.fetchrow('DELETE FROM template_portfolio_generations WHERE id = $1 RETURNING *', generation_id)
        ...
        return {'message': f"Generation '{generation['generation_name']}' deleted successfully", 'details': {'generation_id': generation_id, 'generation_name': generation['generation_name'], 'funds_deleted': funds_count}}
    except HTTPException:
        raise
    except Exception as e:
...
@router.get('/available-portfolio-funds/generation/{generation_id}', response_model=List[dict])
async def get_available_portfolio_funds_by_generation(generation_id: int, db=Depends(get_db)):
    """
    ...
    try:
        ...
        result = await db.fetch('SELECT * FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_id)
        logger.info(f'Query result: {result}')
        return [dict(fund) for fund in result] if result else []
    except Exception as e:
...
async def get_active_template_portfolio_generations(db=Depends(get_db)):
    ...
    try:
        ...
        response = await db.fetch("SELECT id, generation_name, available_portfolio_id, status, created_at, description FROM template_portfolio_generations WHERE status != 'inactive' ORDER BY generation_name")
        if not response:
        ...
        for generation in response:
            ...
            try:
                count_result = await db.fetchval('SELECT COUNT(*) FROM products_list_view WHERE portfolio_id IN (SELECT id FROM portfolios WHERE template_generation_id = $1)', generation_id)
...
async def get_generation_with_funds_batch(generation_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        generation_response = await db.fetchrow('SELECT * FROM template_portfolio_generations WHERE id = $1', generation_id)
        funds_response = await db.fetch('SELECT * FROM available_portfolio_funds WHERE template_portfolio_generation_id = $1', generation_id)
        if not generation_response:
        ...
        result = {'generation': generation_data, 'funds': funds_data, 'template_weightings': template_weightings, 'count': {'generation': 1, 'funds': len(funds_data)}}
...
async def get_products_linked_to_template(portfolio_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        generations_response = await db.fetch('SELECT id FROM template_portfolio_generations WHERE available_portfolio_id = $1', portfolio_id)
        ...
        products_via_portfolio = await db.fetch('SELECT * FROM client_products WHERE portfolio_id IS NOT NULL')
        products_direct = await db.fetch('SELECT * FROM client_products WHERE template_generation_id = ANY($1::int[])', generation_ids)

        async def enrich_product(product):
            if product.get('client_id'):
                client_response = await db.fetchrow('SELECT id, name, advisor FROM client_groups WHERE id = $1', product['client_id'])
                product['client_groups'] = dict(client_response) if client_response else None
            if product.get('provider_id'):
                provider_response = await db.fetchrow('SELECT id, name, theme_color FROM available_providers WHERE id = $1', product['provider_id'])
            ...
            if product.get('portfolio_id'):
                portfolio_response = await db.fetchrow('SELECT id, portfolio_name, template_generation_id FROM portfolios WHERE id = $1', product['portfolio_id'])
        ...
        all_products.sort(key=lambda x: (x.get('client_groups', {}).get('name', '') if x.get('client_groups') else '', x.get('product_name', '')))
        return all_products
    except Exception as e:
