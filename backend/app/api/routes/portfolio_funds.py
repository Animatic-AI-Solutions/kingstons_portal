...
class IRRCache:
    ...
    def _generate_cache_key(self, portfolio_fund_ids: List[int], calculation_date: Optional[str]=None, cash_flows: Optional[List[float]]=None, fund_valuations: Optional[dict]=None) -> str:
        """Generate a unique cache key for IRR calculation inputs."""
        sorted_fund_ids = sorted(portfolio_fund_ids)
        key_data = {'fund_ids': sorted_fund_ids, 'calculation_date': calculation_date or 'latest', 'cash_flows': cash_flows or [], 'fund_valuations': fund_valuations or {}}
        key_string = json.dumps(key_data, sort_keys=True)
        cache_key = hashlib.sha256(key_string.encode()).hexdigest()
        return cache_key

    async def get(self, portfolio_fund_ids: List[int], calculation_date: Optional[str]=None, cash_flows: Optional[List[float]]=None, fund_valuations: Optional[dict]=None) -> Optional[dict]:
    ...
    async def set(self, portfolio_fund_ids: List[int], result: dict, calculation_date: Optional[str]=None, cash_flows: Optional[List[float]]=None, fund_valuations: Optional[dict]=None, ttl_minutes: Optional[int]=None) -> None:
        """Store IRR calculation result in cache."""
        async with self._lock:
            cache_key = self._generate_cache_key(portfolio_fund_ids, calculation_date, cash_flows, fund_valuations)
            ttl = timedelta(minutes=ttl_minutes or self._default_ttl.total_seconds() / 60)
            self._cache[cache_key] = {'data': result, 'created_at': datetime.now(), 'expires_at': datetime.now() + ttl, 'fund_ids': portfolio_fund_ids, 'calculation_date': calculation_date}
    ...
    async def get_stats(self) -> dict:
        ...
        async with self._lock:
            ...
            for cached_item in self._cache.values():
                if current_time > cached_item['expires_at']:
                    expired_entries += 1
            return {'total_entries': total_entries, 'active_entries': total_entries - expired_entries, 'expired_entries': expired_entries, 'cache_hit_potential': f'{(total_entries - expired_entries) / max(total_entries, 1) * 100:.1f}%'}
...
def calculate_excel_style_irr(dates, amounts, guess=0.02):
    ...
    try:
        ...
        if all((d == dates[0] for d in dates)):
            ...
            return {'period_irr': simple_return, 'days_in_period': 0, 'is_simple_return': True}
        start_date = dates[0]
        end_date = dates[-1]
        now = datetime.now()
        ...
        if (end_date - start_date).days < 1:
            ...
            if total_outflow == 0:
                ...
                raise ValueError(error_msg)
            simple_return = total_inflow / abs(total_outflow) - 1
            return {'period_irr': simple_return, 'days_in_period': (end_date - start_date).days, 'is_simple_return': True}
        ...
        if total_months < 1:
            ...
            return {'period_irr': annualized_return, 'days_in_period': days, 'is_simple_return': True}
        ...
        for cash_flow_date, amount in zip(dates, amounts):
            ...
            if month_index < 0 or month_index >= len(monthly_amounts):
                ...
                logger.error(error_msg)
                raise ValueError(error_msg)
            monthly_amounts[month_index] += amount
        ...
        return {'period_irr': annualized_irr, 'days_in_period': days_in_period}
...
@router.get('/portfolio-funds', response_model=List[PortfolioFund])
async def get_portfolio_funds(skip: int=Query(0, ge=0, description='Number of records to skip for pagination'), limit: int=Query(100000, ge=1, le=100000, description='Max number of records to return'), portfolio_id: Optional[int]=None, available_funds_id: Optional[int]=None, select: Optional[str]='*', db=Depends(get_db)):
    """
...
@router.post('/portfolio-funds', response_model=PortfolioFund)
async def create_portfolio_fund(portfolio_fund: PortfolioFundCreate, db=Depends(get_db)):
    """
    ...
    try:
        ...
        portfolio_fund_data = {'portfolio_id': portfolio_fund.portfolio_id, 'available_funds_id': portfolio_fund.available_funds_id, 'target_weighting': to_serializable(portfolio_fund.target_weighting), 'start_date': portfolio_fund.start_date, 'end_date': portfolio_fund.end_date if portfolio_fund.end_date else None, 'amount_invested': portfolio_fund.amount_invested, 'status': portfolio_fund.status}
        ...
        if portfolio_fund.target_weighting is not None:
            try:
                ...
                update_result = await db.fetchrow('UPDATE portfolio_funds SET target_weighting = $1 WHERE id = $2 RETURNING *', float(portfolio_fund.target_weighting), created_fund_id)
                if update_result:
...
async def get_latest_fund_irrs(fund_ids: str=Query(..., description='Comma-separated list of portfolio fund IDs'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if result:
            for irr_record in result:
                ...
                fund_irrs.append({'fund_id': irr_dict['fund_id'], 'irr_result': irr_dict['irr_result'], 'irr_date': irr_dict['date']})
        return {'fund_irrs': fund_irrs, 'count': len(fund_irrs), 'requested_funds': len(fund_id_list), 'source': 'stored'}
...
@router.get('/portfolio-funds/{portfolio_fund_id}', response_model=PortfolioFund)
...
@router.patch('/portfolio-funds/{portfolio_fund_id}', response_model=PortfolioFund)
async def update_portfolio_fund(portfolio_fund_id: int, portfolio_fund_update: PortfolioFundUpdate, db=Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            return {'id': updated_fund.get('id'), 'portfolio_id': updated_fund.get('portfolio_id'), 'available_funds_id': updated_fund.get('available_funds_id'), 'target_weighting': float(updated_fund.get('target_weighting', 0)) if updated_fund.get('target_weighting') is not None else None, 'amount_invested': float(updated_fund.get('amount_invested', 0)) if updated_fund.get('amount_invested') is not None else None, 'status': updated_fund.get('status', 'active'), 'start_date': updated_fund.get('start_date'), 'end_date': updated_fund.get('end_date')}
    except HTTPException:
        raise
    except Exception as e:
...
@router.delete('/portfolio-funds/{portfolio_fund_id}', response_model=dict)
async def delete_portfolio_fund(portfolio_fund_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        return {'message': f'Portfolio fund with ID {portfolio_fund_id} deleted successfully', 'details': {'irr_values_deleted': irr_count, 'activity_logs_deleted': activity_count}}
    except Exception as e:
        logger.error(f'Error deleting portfolio fund: {str(e)}')
        raise HTTPException(status_code=500, detail=f'Database error: {str(e)}')

@router.get('/portfolio-funds/{portfolio_fund_id}/latest-irr', response_model=dict)
async def get_latest_irr(portfolio_fund_id: int, db=Depends(get_db)):
    ...
    try:
        ...
        if not irr_result:
            ...
            return {'portfolio_fund_id': portfolio_fund_id, 'irr': 0, 'calculation_date': None, 'fund_valuation_id': None}
        ...
        if irr_value is None:
            ...
            try:
                ...
                irr_value = 0.0
        response = {'portfolio_fund_id': portfolio_fund_id, 'irr': irr_value, 'irr_decimal': irr_value / 100 if irr_value != 0 else 0.0, 'calculation_date': latest_irr['date'], 'fund_valuation_id': latest_irr.get('fund_valuation_id')}
...
@router.post('/portfolio-funds/{portfolio_fund_id}/recalculate-all-irr', response_model=dict)
async def recalculate_all_irr_values(portfolio_fund_id: int, db=Depends(get_db)):
    """
    ...
    try:
        ...
        if not existing_irr_values:
            ...
            return {'portfolio_fund_id': portfolio_fund_id, 'message': 'No existing IRR values to recalculate. Skipping IRR calculation.', 'updated_count': 0}
        update_count = 0
        for irr_record in existing_irr_values:
            try:
                ...
                if valuation_amount < 0:
                    ...
                    continue
                irr_result = await calculate_single_portfolio_fund_irr(portfolio_fund_id=portfolio_fund_id, irr_date=valuation_date.strftime('%Y-%m-%d'), db=db)
        ...
        return {'portfolio_fund_id': portfolio_fund_id, 'message': f'IRR values recalculated successfully', 'updated_count': update_count}
    except Exception as e:
...
@router.get('/portfolio-funds/{portfolio_fund_id}/irr-values', response_model=List[dict])
    ...
    try:
        ...
        if irr_values:
            for irr_record in irr_values:
                irr = dict(irr_record)
                formatted_irr = {'id': irr['id'], 'date': irr['date'], 'irr': float(irr['irr_result']), 'fund_id': irr['fund_id'], 'created_at': irr['created_at'], 'fund_valuation_id': irr['fund_valuation_id']}
...
@router.post('/portfolio-funds/batch/irr-values', response_model=dict)
async def get_batch_irr_values(fund_ids: List[int]=Body(..., description='List of portfolio fund IDs to fetch IRR values for'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if irr_values:
            for irr_record in irr_values:
                irr = dict(irr_record)
                fund_id = irr['fund_id']
                formatted_irr = {'id': irr['id'], 'date': irr['date'], 'irr': float(irr['irr_result']), 'fund_id': fund_id, 'created_at': irr['created_at'], 'fund_valuation_id': irr['fund_valuation_id']}
...
@router.post('/portfolio-funds/batch/irr-values-by-date', response_model=dict)
async def get_batch_irr_values_by_date(fund_ids: List[int]=Body(..., description='List of portfolio fund IDs to fetch IRR values for'), target_month: int=Body(..., description='Target month (1-12)'), target_year: int=Body(..., description='Target year (e.g., 2024)'), db=Depends(get_db)):
    """
...
@router.post('/portfolio-funds/aggregated-irr-history', response_model=dict)
async def get_aggregated_irr_history(fund_ids: List[int]=Body(None, description='List of portfolio fund IDs to fetch IRR history for'), portfolio_id: Optional[int]=Body(None, description='Optional portfolio ID to filter funds'), include_fund_details: bool=Body(True, description='Include detailed fund information'), include_portfolio_irr: bool=Body(True, description='Include portfolio-level IRR calculations'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if portfolio_id and (not fund_ids):
            ...
            if portfolio_funds:
                ...
                return {'columns': [], 'funds': [], 'portfolio_info': None, 'portfolio_irr': {} if include_portfolio_irr else None}
            portfolio_info = None
            try:
        ...
        if available_fund_ids:
            ...
            if available_funds:
                available_funds_map = {dict(fund)['id']: dict(fund) for fund in available_funds}
        for fund_id, portfolio_fund in portfolio_fund_map.items():
            ...
            fund_details[fund_id] = {'id': fund_id, 'name': available_fund.get('fund_name', 'Unknown Fund'), 'risk_level': available_fund.get('risk_level', None), 'fund_type': available_fund.get('fund_type', None), 'target_weighting': portfolio_fund.get('target_weighting', 0), 'start_date': portfolio_fund.get('start_date', None), 'status': portfolio_fund.get('status', 'active'), 'available_fund': {'id': available_fund_id, 'name': available_fund.get('fund_name', 'Unknown Fund'), 'description': available_fund.get('description', None), 'provider': available_fund.get('provider', None)}}
        irr_values = await db.fetch('\n            SELECT * FROM portfolio_fund_irr_values \n            WHERE fund_id = ANY($1::int[]) \n            ORDER BY date\n        ', fund_ids)
        if not irr_values:
            logger.info('No IRR values found for the specified funds')
            return {'columns': [], 'funds': [{'id': fund_id, 'name': fund_details.get(fund_id, {}).get('name', 'Unknown Fund'), 'details': fund_details.get(fund_id, {}) if include_fund_details else None, 'values': {}} for fund_id in fund_ids if fund_id in fund_details], 'portfolio_info': portfolio_info, 'portfolio_irr': {} if include_portfolio_irr else None}
        months_set = set()
        funds_data = {}
        for irr_record in irr_values:
            irr = dict(irr_record)
            ...
            if fund_id not in funds_data:
                funds_data[fund_id] = {'id': fund_id, 'name': fund_details.get(fund_id, {}).get('name', 'Unknown Fund'), 'details': fund_details.get(fund_id, {}) if include_fund_details else None, 'values': {}}
        ...
        for fund_id in fund_ids:
            if fund_id in fund_details and fund_id not in funds_data:
                funds_data[fund_id] = {'id': fund_id, 'name': fund_details[fund_id]['name'], 'details': fund_details.get(fund_id, {}) if include_fund_details else None, 'values': {}}
        funds_list = list(funds_data.values())
        funds_list.sort(key=lambda f: f['name'])
        result = {'columns': months_list, 'funds': funds_list, 'portfolio_info': portfolio_info, 'portfolio_irr': portfolio_irr_values if include_portfolio_irr else None}
...
@router.patch('/irr-values/{irr-value-id}', response_model=dict)
async def update_irr_value(irr_value_id: int, data: dict=Body(..., description='IRR value update data'), db=Depends(get_db)):
    """
...
@router.delete('/irr-values/{irr-value-id}', status_code=204)
...
async def create_irr_value(fund_id: int=Body(..., description='Portfolio fund ID'), irr_result: float=Body(..., description='IRR result as percentage (e.g., 5.25 for 5.25%)'), date: str=Body(..., description='Date for the IRR calculation in YYYY-MM-DD format'), fund_valuation_id: Optional[int]=Body(None, description='Optional fund valuation ID reference'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        irr_value_data = {'fund_id': fund_id, 'irr_result': float(round(irr_result, 2)), 'date': date, 'fund_valuation_id': fund_valuation_id}
        if existing_irr:
            ...
            if result:
                ...
                return {'action': 'updated', 'irr_value': updated_record}
            else:
                raise HTTPException(status_code=500, detail='Failed to update IRR value')
        else:
            ...
            if result:
                ...
                return {'action': 'created', 'irr_value': created_record}
            else:
...
async def calculate_portfolio_fund_irr(portfolio_fund_id: int, month: int, year: int, valuation: float=None, db=None, update_only: bool=False):
    """
    ...
    try:
        ...
        try:
            ...
            irr_value_data = {'fund_id': portfolio_fund_id, 'irr_result': float(round(annual_irr_percent, 2)), 'date': calculation_date.isoformat(), 'fund_valuation_id': fund_valuation_id}
            if update_only:
                ...
                if not existing_irr:
                    ...
                    return {'status': 'skipped', 'message': 'No existing IRR record to update', 'portfolio_fund_id': portfolio_fund_id, 'irr': round(annual_irr_percent, 2), 'irr_decimal': annual_irr, 'calculation_date': calculation_date.isoformat()}
                else:
            ...
            result = {'portfolio_fund_id': portfolio_fund_id, 'irr': float(round(annual_irr_percent, 2)), 'irr_decimal': float(annual_irr), 'calculation_date': calculation_date.isoformat(), 'fund_valuation_id': fund_valuation_id}
...
@router.post('/portfolio-funds/multiple/irr', response_model=dict)
async def calculate_multiple_portfolio_funds_irr(portfolio_fund_ids: List[int]=Body(..., description='List of portfolio fund IDs to include in IRR calculation'), irr_date: Optional[str]=Body(None, description='Date for IRR calculation in YYYY-MM-DD format (defaults to latest valuation date)'), bypass_cache: bool=Body(False, description='Bypass cache for fresh calculation (used during cascade operations)'), store_result: bool=Body(True, description='Store the calculated IRR in the database (set to False for display-only calculations)'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if not bypass_cache:
            cached_result = await _irr_cache.get(portfolio_fund_ids=portfolio_fund_ids, calculation_date=irr_date.isoformat() if isinstance(irr_date, date) else irr_date)
        ...
        if funds_with_valuations == 0:
            ...
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_ids': portfolio_fund_ids, 'total_valuation': 0.0, 'fund_valuations': fund_valuations, 'cash_flows_count': 0, 'period_start': irr_date_obj.isoformat(), 'period_end': irr_date_obj.isoformat(), 'days_in_period': 0, 'note': f'No valuations found as of {irr_date_obj} - IRR set to 0%'}
        portfolio_fund_ids_with_valuations = [fund_id for fund_id in portfolio_fund_ids if fund_valuations.get(fund_id) is not None]
        excluded_funds = [fund_id for fund_id in portfolio_fund_ids if fund_valuations.get(fund_id) is None]
        if excluded_funds:
        ...
        if not portfolio_fund_ids:
            ...
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_ids': [], 'total_valuation': 0.0, 'fund_valuations': {}, 'cash_flows_count': 0, 'period_start': irr_date_obj.isoformat(), 'period_end': irr_date_obj.isoformat(), 'days_in_period': 0, 'note': f'No funds with valuations available for {irr_date_obj} - IRR set to 0%'}
        ...
        if os.getenv('DEBUG_IRR_VERBOSE', 'False').lower() == 'true':
            ...
            for fund_id in portfolio_fund_ids:
                ...
                for activity in fund_activities[:3]:
        ...
        for activity in activities:
            ...
            try:
                if any((keyword in activity_type.lower() for keyword in ['investment'])):
                    cash_flows_by_date[activity_start_key] -= amount
        ...
        if total_valuation > 0:
            ...
            cash_flows_by_date[next_month_key] += total_valuation
        cash_flows = cash_flows_by_date
        if len(activities) == 0:
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_ids': portfolio_fund_ids, 'total_valuation': total_valuation, 'fund_valuations': fund_valuations, 'cash_flows_count': 0, 'period_start': irr_date_obj.isoformat(), 'period_end': irr_date_obj.isoformat(), 'days_in_period': 0, 'note': 'No activities found - IRR set to 0%'}
        total_cash_flow = sum((abs(amount) for amount in cash_flows.values()))
        if total_cash_flow < 0.01:
            ...
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_ids': portfolio_fund_ids, 'total_valuation': total_valuation, 'fund_valuations': fund_valuations, 'cash_flows_count': len(cash_flows), 'period_start': irr_date_obj.isoformat(), 'period_end': irr_date_obj.isoformat(), 'days_in_period': 0, 'note': 'All cash flows are effectively zero - IRR set to 0%'}
        logger.error(f'')
        logger.error(f"{'=' * 80}")
        ...
        result = {'success': True, 'irr_percentage': round(irr_decimal * 100, 1), 'irr_decimal': irr_decimal, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_ids': portfolio_fund_ids, 'total_valuation': total_valuation, 'fund_valuations': fund_valuations, 'cash_flows_count': len(cash_flows), 'period_start': min(cash_flows.keys()).isoformat(), 'period_end': max(cash_flows.keys()).isoformat(), 'days_in_period': days_in_period}
        ...
        await _irr_cache.set(portfolio_fund_ids=portfolio_fund_ids, calculation_date=irr_date_obj.isoformat() if irr_date_obj else None, result=result)
        if store_result:
...
@router.post('/portfolio-funds/{portfolio_fund_id}/irr', response_model=dict)
async def calculate_single_portfolio_fund_irr(portfolio_fund_id: int, irr_date: Optional[str]=Body(None, description='Date for IRR calculation in YYYY-MM-DD format (defaults to latest valuation date)'), bypass_cache: bool=Body(False, description='Bypass cache for fresh calculation (used during cascade operations)'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if not bypass_cache:
            cached_result = await _irr_cache.get(portfolio_fund_ids=[portfolio_fund_id], calculation_date=irr_date.isoformat() if isinstance(irr_date, date) else irr_date)
            if cached_result is not None:
        ...
        if valuation_amount == 0:
            ...
            next_month = valuation_month_key.replace(day=28) + timedelta(days=4)
            next_month_key = next_month.replace(day=1)
            if next_month_key not in cash_flows:
                cash_flows[next_month_key] = 0.0
            cash_flows[next_month_key] += valuation_amount
        ...
        if len(activities) == 0:
            ...
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_obj.isoformat(), 'portfolio_fund_id': portfolio_fund_id, 'valuation_amount': valuation_amount, 'cash_flows_count': 0, 'period_start': irr_date_obj.isoformat(), 'period_end': irr_date_obj.isoformat(), 'days_in_period': 0, 'note': 'No activities found - IRR set to 0%'}
        if len(cash_flows) == 0:
            ...
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_iso, 'portfolio_fund_id': portfolio_fund_id, 'valuation_amount': valuation_amount, 'cash_flows_count': 0, 'period_start': irr_date_iso, 'period_end': irr_date_iso, 'days_in_period': 0, 'note': 'No cash flows after processing (zero valuation) - IRR set to 0%'}
        total_cash_flow = sum((abs(amount) for amount in cash_flows.values()))
        if total_cash_flow < 0.01:
            ...
            irr_date_iso = irr_date_obj.isoformat()
            return {'success': True, 'irr_percentage': 0.0, 'irr_decimal': 0.0, 'calculation_date': irr_date_iso, 'portfolio_fund_id': portfolio_fund_id, 'valuation_amount': valuation_amount, 'cash_flows_count': len(cash_flows), 'period_start': irr_date_iso, 'period_end': irr_date_iso, 'days_in_period': 0, 'note': 'All cash flows are effectively zero - IRR set to 0%'}
        ...
        final_result = {'success': True, 'irr_percentage': irr_percentage, 'irr_decimal': irr_decimal, 'calculation_date': irr_date_iso, 'portfolio_fund_id': portfolio_fund_id, 'valuation_amount': valuation_amount, 'cash_flows_count': len(cash_flows), 'period_start': sorted_months[0].isoformat() if sorted_months else irr_date_iso, 'period_end': sorted_months[-1].isoformat() if sorted_months else irr_date_iso, 'days_in_period': days_in_period}
        cash_flow_values = [cash_flows[month] for month in sorted(cash_flows.keys())]
        cash_flow_values = [0.0 if abs(amount) < 0.01 else amount for amount in cash_flow_values]
        await _irr_cache.set(portfolio_fund_ids=[portfolio_fund_id], result=final_result, calculation_date=irr_date_obj.isoformat() if irr_date_obj else None, cash_flows=cash_flow_values, fund_valuations={portfolio_fund_id: valuation_amount}, ttl_minutes=30)
        cash_flow_values = [cash_flows[month] for month in sorted(cash_flows.keys())]
        cash_flow_values = [0.0 if abs(amount) < 0.01 else amount for amount in cash_flow_values]
        await _irr_cache.set(portfolio_fund_ids=[portfolio_fund_id], result=final_result, calculation_date=irr_date_obj.isoformat() if irr_date_obj else None, cash_flows=cash_flow_values, fund_valuations={portfolio_fund_id: valuation_amount}, ttl_minutes=30)
...
async def get_batch_fund_valuations(request: BatchValuationsRequest, db=Depends(get_db)):
    """
    ...
    try:
        ...
        for valuation_record_row in result:
            ...
            fund_id = valuation_record['portfolio_fund_id']
            if fund_id not in seen_funds:
                fund_valuations[fund_id] = {'valuation': valuation_record['valuation'], 'valuation_date': valuation_record['valuation_date']}
                seen_funds.add(fund_id)
        for fund_id in fund_ids:
            if fund_id not in fund_valuations:
                fund_valuations[fund_id] = {'valuation': None, 'valuation_date': None}
        logger.info(f'✅ Batch valuation fetch complete: {len(fund_valuations)} funds processed')
        return {'fund_valuations': fund_valuations, 'count': len(fund_valuations), 'requested_funds': len(fund_ids), 'source': 'batch_optimized'}
    except Exception as e:
...
async def get_batch_historical_fund_valuations(request: BatchHistoricalValuationsRequest, db=Depends(get_db)):
    """
    ...
    try:
        ...
        for valuation_record_row in result:
            ...
            fund_historical_valuations[fund_id].append({'valuation': valuation_record['valuation'], 'valuation_date': valuation_record['valuation_date']})
            total_valuations += 1
        for fund_id in fund_ids:
            if fund_id not in fund_historical_valuations:
                fund_historical_valuations[fund_id] = []
        ...
        return {'fund_historical_valuations': fund_historical_valuations, 'count': len(fund_historical_valuations), 'total_valuations': total_valuations, 'requested_funds': len(fund_ids), 'source': 'batch_historical_optimized'}
    except Exception as e:
...
async def get_irr_cache_stats():
    ...
    try:
        ...
        return {'success': True, 'cache_stats': stats, 'timestamp': datetime.now().isoformat()}
    except Exception as e:
...
async def clear_expired_irr_cache_entries():
    ...
    try:
        ...
        return {'success': True, 'cleared_entries': cleared_count, 'timestamp': datetime.now().isoformat()}
    except Exception as e:
...
async def invalidate_irr_cache_for_funds(fund_ids: List[int]=Body(..., description='List of portfolio fund IDs to invalidate cache for')):
    """
    ...
    try:
        ...
        return {'success': True, 'invalidated_entries': invalidated_count, 'fund_ids': fund_ids, 'timestamp': datetime.now().isoformat()}
    except Exception as e:
        logger.error(f'Error invalidating IRR cache for funds {fund_ids}: {str(e)}')
        raise HTTPException(status_code=500, detail=f'Failed to invalidate cache: {str(e)}')

@router.post('/portfolio-funds/{portfolio_fund_id}/recalculate-irr', response_model=dict)
async def recalculate_irr_after_activity(portfolio_fund_id: int, request: dict, db=Depends(get_db)):
    """
    ...
    try:
        ...
        result = await recalculate_irr_after_activity_change(portfolio_fund_id=portfolio_fund_id, db=db, activity_date=activity_date)
        return {'success': True, 'portfolio_fund_id': portfolio_fund_id, 'activity_date': activity_date, 'recalculation_result': result}
    except Exception as e:
...
@router.get('/portfolio-funds/enhanced/{product_id}', response_model=List[dict])
async def get_enhanced_portfolio_funds(product_id: int, db=Depends(get_db)):
    """
    ...
    try:
        ...
        result = []
        for row in rows:
            fund_data = {'portfolio_fund_id': row['portfolio_fund_id'], 'fund_id': row['fund_id'], 'fund_name': row['fund_name'], 'isin_number': row['isin_number'], 'risk_factor': row['risk_factor'], 'fund_cost': float(row['fund_cost']) if row['fund_cost'] else None, 'amount_invested': float(row['amount_invested']) if row['amount_invested'] else 0.0, 'target_weighting': float(row['target_weighting']) if row['target_weighting'] else None, 'fund_status': row['fund_status'], 'fund_start_date': row['fund_start_date'].isoformat() if row['fund_start_date'] else None, 'fund_end_date': row['fund_end_date'].isoformat() if row['fund_end_date'] else None, 'current_valuation': float(row['current_valuation']) if row['current_valuation'] else None, 'latest_valuation_date': row['latest_valuation_date'].isoformat() if row['latest_valuation_date'] else None, 'latest_irr': float(row['latest_irr']) if row['latest_irr'] else None, 'latest_irr_date': row['latest_irr_date'].isoformat() if row['latest_irr_date'] else None, 'product_id': row['product_id'], 'product_name': row['product_name'], 'client_name': row['client_name'], 'provider_name': row['provider_name'], 'provider_color': row['provider_color']}
...
@router.post('/portfolio-funds/multiple/historical-irr')
async def calculate_multiple_portfolio_funds_historical_irr(portfolio_fund_ids: List[int]=Body(..., description='List of portfolio fund IDs to include in historical IRR calculation'), historical_dates: List[str]=Body(..., description='List of dates for historical IRR calculation in YYYY-MM-DD format'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        for original_date_str, date_obj in validated_dates:
            ...
            if funds_with_valuations == 0 or len(activities) == 0:
                ...
                historical_irr_results[original_date_str] = {'irr_percentage': 0.0, 'note': f'No data available as of {original_date_str}'}
                continue
            cash_flows_by_date = {}
            for activity in activities:
                activity_timestamp = activity['activity_timestamp']
            ...
            if total_cash_flow < 0.01:
                ...
                historical_irr_results[original_date_str] = {'irr_percentage': 0.0, 'note': f'All cash flows are effectively zero as of {original_date_str}'}
                continue
            ...
            try:
                ...
                historical_irr_results[original_date_str] = {'irr_percentage': round(irr_percentage, 2), 'irr_decimal': round(irr_decimal, 4) if irr_decimal is not None else 0.0, 'total_valuation': total_valuation, 'activities_count': len(activities), 'cash_flows_count': len(cash_flows_by_date), 'calculation_date': date_obj.isoformat(), 'days_in_period': irr_result.get('days_in_period', 0) if irr_result else 0}
                ...
                historical_irr_results[original_date_str] = {'irr_percentage': 0.0, 'error': f'IRR calculation failed: {str(irr_error)}'}
        return {'success': True, 'portfolio_fund_ids': portfolio_fund_ids, 'historical_dates': historical_dates, 'historical_irr_results': historical_irr_results, 'total_dates_calculated': len(historical_irr_results)}
    except Exception as e:
