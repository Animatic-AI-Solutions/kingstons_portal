...
async def should_recalculate_portfolio_irr(portfolio_id: int, valuation_date: str, db) -> bool:
    ...
    """
    try:
        all_portfolio_funds = await db.fetch('SELECT id, status FROM portfolio_funds WHERE portfolio_id = $1', portfolio_id)
        ...
        for fund_id in active_funds:
            fund_valuation_check = await db.fetch('SELECT id FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 AND valuation_date >= $2 AND valuation_date <= $3', fund_id, date_start, date_end)
...
@router.post('/fund-valuations', response_model=FundValuation)
async def create_fund_valuation(fund_valuation: FundValuationCreate, db=Depends(get_db)):
    """
    ...
    try:
        ...
        existing_valuation = await db.fetchrow('SELECT * FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 AND valuation_date = $2', fund_valuation.portfolio_fund_id, fund_valuation.valuation_date)
        if existing_valuation:
            ...
            updated_valuation = await db.fetchrow('UPDATE portfolio_fund_valuations SET valuation = $1 WHERE id = $2 RETURNING *', valuation_amount, existing_id)
            if updated_valuation:
                ...
                try:
                    ...
                    irr_result = await irr_service.handle_fund_valuation_creation_edit(fund_valuation.portfolio_fund_id, valuation_date)
        ...
        created_valuation = await db.fetchrow('INSERT INTO portfolio_fund_valuations (portfolio_fund_id, valuation_date, valuation) VALUES ($1, $2, $3) RETURNING *', fund_valuation.portfolio_fund_id, fund_valuation.valuation_date, valuation_amount)
...
@router.get('/fund-valuations', response_model=List[FundValuation])
async def get_fund_valuations(portfolio_fund_id: Optional[int]=Query(None), db=Depends(get_db)):
    """
    Get all fund valuations, optionally filtered by portfolio_fund_id.
    """
    try:
        if portfolio_fund_id is not None:
            result = await db.fetch('SELECT * FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 ORDER BY valuation_date DESC', portfolio_fund_id)
        else:
...
@router.get('/fund-valuations/latest/{portfolio_fund_id}', response_model=FundValuation)
async def get_latest_fund_valuation(portfolio_fund_id: int, db=Depends(get_db)):
    """
    Get the latest fund valuation for a specific portfolio fund.
    """
    try:
        result = await db.fetchrow('SELECT * FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 ORDER BY valuation_date DESC LIMIT 1', portfolio_fund_id)
        if not result:
...
@router.get('/all-latest-fund-valuations', response_model=List[LatestFundValuationViewItem])
async def get_all_latest_fund_valuations_from_view(db=Depends(get_db)):
    """
...
@router.get('/fund-valuations/exists/{portfolio_fund_id}')
async def check_fund_valuation_exists(portfolio_fund_id: int, year: int=Query(..., ge=1900, le=2100), month: int=Query(..., ge=1, le=12), db=Depends(get_db)):
    """
    ...
    try:
        ...
        result = await db.fetchrow('SELECT id FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 AND valuation_date >= $2 AND valuation_date < $3 LIMIT 1', portfolio_fund_id, first_day, next_month)
        return {'exists': bool(result)}
    except Exception as e:
        logger.error(f'Error checking fund valuation existence: {str(e)}')
        raise HTTPException(status_code=500, detail=f'Database error: {str(e)}')

@router.get('/fund-valuations/{valuation_id}', response_model=FundValuation)
async def get_fund_valuation(valuation_id: int, db=Depends(get_db)):
    """
...
@router.patch('/fund-valuations/{valuation_id}', response_model=FundValuation)
async def update_fund_valuation(valuation_id: int, fund_valuation: FundValuationUpdate, db=Depends(get_db)):
    """
    ...
    try:
        ...
        if hasattr(fund_valuation, 'valuation') and isinstance(fund_valuation.valuation, str) and (fund_valuation.valuation.strip() == ''):
            ...
            try:
                ...
                return {'message': f'Fund valuation {valuation_id} deleted successfully with IRR cascade', 'valuation_deleted': cascade_result.get('valuation_deleted', False), 'fund_irr_deleted': cascade_result.get('fund_irr_deleted', False), 'portfolio_irr_deleted': cascade_result.get('portfolio_irr_deleted', False), 'portfolio_valuation_deleted': cascade_result.get('portfolio_valuation_deleted', False), 'completeness_maintained': cascade_result.get('completeness_maintained', False)}
            except HTTPException:
                raise
            except Exception as e:
        ...
        updated_valuation = await db.fetchrow(f'UPDATE portfolio_fund_valuations SET {set_clause} WHERE id = ${param_count} RETURNING *', *update_params)
        ...
        try:
            ...
            irr_service = IRRCascadeService(db)
            irr_result = await irr_service.handle_fund_valuation_creation_edit(portfolio_fund_id, valuation_date)
            if irr_result.get('success'):
                logger.info(f'📈 [IRR CASCADE] ✅ IRR calculation completed: {irr_result}')
            else:
                logger.warning(f'📈 [IRR CASCADE] ⚠️ IRR calculation had issues: {irr_result}')
...
@router.delete('/fund-valuations/{valuation_id}', response_model=dict)
async def delete_fund_valuation(valuation_id: int, db=Depends(get_db)):
    """
    ...
    try:
        ...
        try:
            ...
            return {'message': response_message, 'status': 'success', 'valuation_deleted': cascade_result.get('valuation_deleted', False), 'fund_irr_deleted': cascade_result.get('fund_irr_deleted', False), 'portfolio_irr_deleted': cascade_result.get('portfolio_irr_deleted', False), 'portfolio_valuation_deleted': cascade_result.get('portfolio_valuation_deleted', False), 'completeness_maintained': cascade_result.get('completeness_maintained', False), 'affected_date': cascade_result.get('date'), 'cascade_details': cascade_details}
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f'🗑️ [IRR CASCADE] ❌ Unexpected error in cascade deletion: {str(e)}')
...
@router.get('/fund-valuations/latest-date')
...
@router.get('/latest-fund-valuations')
async def get_latest_fund_valuations(portfolio_fund_id: Optional[int]=Query(None, description='Filter by portfolio fund ID'), skip: int=Query(0, ge=0, description='Number of records to skip'), limit: int=Query(100000, ge=1, le=100000, description='Number of records to return'), db=Depends(get_db)):
    """
    Get the latest fund valuations from the latest_portfolio_fund_valuations view.
    """
    try:
        if portfolio_fund_id is not None:
            result = await db.fetch('SELECT * FROM latest_portfolio_fund_valuations WHERE portfolio_fund_id = $1', portfolio_fund_id)
        else:
            result = await db.fetch('SELECT * FROM latest_portfolio_fund_valuations')
...
@router.post('/fund-valuations/test-cascade-deletion')
async def test_cascade_deletion(portfolio_id: int=Query(..., description='Portfolio ID to test cascade deletion with'), test_date: str=Query(..., description='Date to test (YYYY-MM-DD format)'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        active_funds = await db.fetch("SELECT id, available_funds_id FROM portfolio_funds WHERE portfolio_id = $1 AND status = 'active'", portfolio_id)
        if not active_funds:
            return {'test_status': 'completed', 'error': f'No active funds found in portfolio {portfolio_id}', 'portfolio_id': portfolio_id, 'test_date': test_date}
        fund_ids = [f['id'] for f in active_funds]
        logger.info(f'🧪 Found {len(fund_ids)} active funds: {fund_ids}')
        current_valuations = await db.fetch('SELECT id, portfolio_fund_id, valuation FROM portfolio_fund_valuations WHERE portfolio_fund_id = ANY($1::int[]) AND valuation_date = $2', fund_ids, test_date)
        ...
        portfolio_irr = await db.fetch('SELECT * FROM portfolio_irr_values WHERE portfolio_id = $1 AND date = $2', portfolio_id, test_date)
        portfolio_valuation = await db.fetch('SELECT * FROM portfolio_valuations WHERE portfolio_id = $1 AND valuation_date = $2', portfolio_id, test_date)
        simulation_results = []
        for fund_valuation in current_valuations:
            fund_id = fund_valuation['portfolio_fund_id']
            valuation_id = fund_valuation['id']
            irr_service = IRRCascadeService(db)
            would_be_complete = await irr_service._check_portfolio_completeness_after_deletion(portfolio_id, test_date, fund_id)
            simulation_results.append({'fund_id': fund_id, 'valuation_id': valuation_id, 'current_valuation': fund_valuation['valuation'], 'would_be_complete_after_deletion': would_be_complete, 'cascade_actions': {'delete_portfolio_irr': not would_be_complete and len(portfolio_irr) > 0, 'delete_portfolio_valuation': not would_be_complete and len(portfolio_valuation) > 0}})
        return {'test_status': 'completed', 'portfolio_id': portfolio_id, 'test_date': test_date, 'current_state': {'total_active_funds': len(fund_ids), 'funds_with_valuations': len(funds_with_valuations), 'funds_without_valuations': len(funds_without_valuations), 'is_complete': is_currently_complete, 'has_portfolio_irr': len(portfolio_irr) > 0, 'has_portfolio_valuation': len(portfolio_valuation) > 0}, 'fund_details': {'active_fund_ids': fund_ids, 'funds_with_valuations': funds_with_valuations, 'funds_without_valuations': funds_without_valuations}, 'deletion_simulations': simulation_results, 'recommendation': {'issue_detected': any((not sim['would_be_complete_after_deletion'] for sim in simulation_results)), 'explanation': 'If any fund valuation is deleted and portfolio becomes incomplete, cascading deletion should remove portfolio IRR and valuation'}}
    except Exception as e:
        logger.error(f'Error in cascade deletion test: {str(e)}')
        import traceback
        logger.error(f'Traceback: {traceback.format_exc()}')
        return {'test_status': 'failed', 'error': str(e), 'portfolio_id': portfolio_id, 'test_date': test_date}

@router.get('/fund-valuations/debug-api-calls')
async def debug_api_calls(portfolio_id: int=Query(..., description='Portfolio ID to track API calls for'), db=Depends(get_db)):
    """
    ...
    portfolio_irrs = await db.fetch('SELECT id, irr_result, date, created_at FROM portfolio_irr_values WHERE portfolio_id = $1 ORDER BY date DESC', portfolio_id)
    ...
    return {'success': True, 'portfolio_id': portfolio_id, 'request_time': time.strftime('%H:%M:%S'), 'portfolio_irr_count': len(records), 'records': records}

@router.get('/fund-valuations/debug-portfolio-irrs')
async def debug_portfolio_irrs(portfolio_id: int=Query(..., description='Portfolio ID to check IRRs for'), date: Optional[str]=Query(None, description='Specific date to check (YYYY-MM-DD format), or leave blank for all dates'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        if date:
            ...
            result = await db.fetch('SELECT id, irr_result, date, created_at, portfolio_valuation_id FROM portfolio_irr_values WHERE portfolio_id = $1 AND date = $2 ORDER BY date DESC', portfolio_id, date)
        else:
            result = await db.fetch('SELECT id, irr_result, date, created_at, portfolio_valuation_id FROM portfolio_irr_values WHERE portfolio_id = $1 ORDER BY date DESC', portfolio_id)
        ...
        response = {'success': True, 'portfolio_id': portfolio_id, 'date_filter': date, 'portfolio_irr_count': len(records), 'portfolio_irrs': [{'id': record['id'], 'irr_result': record.get('irr_result'), 'date': record['date'], 'created_at': record.get('created_at'), 'portfolio_valuation_id': record.get('portfolio_valuation_id')} for record in records], 'fund_irr_summary': {date: len(fund_irrs) for date, fund_irrs in fund_irr_by_date.items()} if not date else {date: len(fund_irr_by_date.get(date, []))}}
        return response
    except Exception as e:
