...
async def validate_activity_date_against_product(portfolio_fund_id: int, activity_date: date, db) -> None:
    ...
    try:
        ...
        if activity_year_month < product_start_year_month:
            ...
            raise HTTPException(status_code=400, detail=f'Invalid activity date: {error_msg}')
...
async def recalculate_irr_after_activity_change(portfolio_fund_id: int, db, activity_date: str=None):
    ...
    try:
        ...
        portfolio_fund_result = await db.fetchrow('SELECT portfolio_id FROM portfolio_funds WHERE id = $1', portfolio_fund_id)
        ...
        if not activity_date:
            latest_activity = await db.fetchrow('SELECT activity_timestamp FROM holding_activity_log WHERE portfolio_fund_id = $1 ORDER BY activity_timestamp DESC LIMIT 1', portfolio_fund_id)
            if latest_activity:
        ...
        if cascade_result.get('success'):
            ...
            return {'success': True, 'portfolio_fund_id': portfolio_fund_id, 'portfolio_id': portfolio_id, 'activity_start_date': activity_date, 'recalculated_existing': cascade_result.get('fund_irrs_recalculated', 0), 'new_entries_created': 0, 'portfolio_irr_recalculated': cascade_result.get('portfolio_irrs_recalculated', 0), 'dates_processed': cascade_result.get('dates_processed', 0), 'cascade_service_used': True, 'method': 'targeted_activity_batch'}
        else:
...
async def recalculate_irr_after_historical_change(portfolio_fund_id: int, db, activity_date: str=None):
    ...
    try:
        ...
        if cascade_result.get('success'):
            ...
            return {'success': True, 'portfolio_fund_id': portfolio_fund_id, 'portfolio_id': portfolio_id, 'activity_start_date': activity_date, 'recalculated_existing': cascade_result.get('fund_irrs_recalculated', 0), 'new_entries_created': 0, 'portfolio_irr_recalculated': cascade_result.get('portfolio_irrs_recalculated', 0), 'dates_processed': cascade_result.get('dates_processed', 0), 'cascade_service_used': True, 'method': 'full_historical_recalculation'}
        else:
...
async def find_common_valuation_dates_from_date(fund_ids: List[int], start_date: str, db) -> List[str]:
    ...
    try:
        ...
        fund_statuses = await db.fetch('SELECT id, status FROM portfolio_funds WHERE id = ANY($1::int[])', fund_ids)
        ...
        all_valuations = await db.fetch('SELECT portfolio_fund_id, valuation_date FROM portfolio_fund_valuations WHERE portfolio_fund_id = ANY($1::int[]) AND valuation_date >= $2', fund_ids, start_date)
        ...
        if inactive_funds:
            for fund_id in inactive_funds:
                latest_valuation = await db.fetchrow('SELECT valuation_date FROM portfolio_fund_valuations WHERE portfolio_fund_id = $1 ORDER BY valuation_date DESC LIMIT 1', fund_id)
                if latest_valuation:
        ...
        for valuation in all_valuations:
            ...
            if fund_id in active_funds:
                date = str(valuation['valuation_date']).split('T')[0]
        ...
        for date, active_fund_set in active_fund_dates.items():
            if len(active_fund_set) == len(active_funds):
                ...
                for inactive_fund_id in inactive_funds:
                    has_valuation_on_date = any((v['portfolio_fund_id'] == inactive_fund_id and str(v['valuation_date']).split('T')[0] == date for v in all_valuations))
...
async def calculate_portfolio_valuation_for_date(portfolio_id: int, date: str, db) -> float:
    ...
    try:
        ...
        fund_valuations = await db.fetch('SELECT valuation FROM portfolio_fund_valuations WHERE portfolio_fund_id = ANY($1::int[]) AND valuation_date = $2', fund_ids, date)
...
@router.get('/holding-activity-logs', response_model=List[HoldingActivityLog])
async def get_holding_activity_logs(skip: int=Query(0, ge=0, description='Number of records to skip for pagination'), limit: int=Query(100000, ge=1, le=100000, description='Max number of records to return'), product_id: Optional[int]=None, portfolio_fund_id: Optional[int]=None, portfolio_id: Optional[int]=None, activity_type: Optional[str]=None, from_date: Optional[str]=None, to_date: Optional[str]=None, db=Depends(get_db)):
    """
...
@router.post('/holding-activity-logs', response_model=HoldingActivityLog)
async def create_holding_activity_log(log: HoldingActivityLogCreate, request: Request, db=Depends(get_db), skip_irr_calculation: bool=False):
    """
    ...
    try:
        ...
        created_activity = await db.fetchrow('INSERT INTO holding_activity_log (portfolio_fund_id, product_id, activity_type, activity_timestamp, amount) VALUES ($1, $2, $3, $4, $5) RETURNING *', log.portfolio_fund_id, log.product_id, log.activity_type, activity_datetime, float(log.amount) if log.amount is not None else None)
        ...
        if skip_irr_calculation:
            ...
            try:
                if hasattr(log.activity_timestamp, 'isoformat'):
                    activity_date = log.activity_timestamp.isoformat()
                else:
...
@router.get('/holding-activity-logs/earliest-date')
async def get_earliest_activity_date(product_id: Optional[int]=None, portfolio_fund_id: Optional[int]=None, db=Depends(get_db)):
    """
...
@router.get('/holding-activity-logs/{holding_activity_log_id}', response_model=HoldingActivityLog)
...
@router.patch('/holding-activity-logs/{activity_id}', response_model=HoldingActivityLog)
async def update_holding_activity_log(activity_id: int, holding_activity_log: HoldingActivityLogUpdate, db=Depends(get_db)):
    """
...
@router.delete('/holding-activity-logs/{holding_activity_log_id}')
...
@router.post('/holding-activity-logs/test-sophisticated-irr-recalculation')
async def test_sophisticated_irr_recalculation(portfolio_fund_id: int=Query(..., description='Portfolio fund ID to test IRR recalculation for'), activity_date: Optional[str]=Query(None, description='Activity date to use for testing (YYYY-MM-DD format)'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        portfolio_fund_check = await db.fetchrow('SELECT id, portfolio_id FROM portfolio_funds WHERE id = $1', portfolio_fund_id)
        ...
        existing_irr_before = await db.fetch('SELECT * FROM portfolio_fund_irr_values WHERE fund_id = $1 AND date >= $2', portfolio_fund_id, activity_date)
        existing_portfolio_irr_before = await db.fetch('SELECT * FROM portfolio_irr_values WHERE portfolio_id = $1 AND date >= $2', portfolio_id, activity_date)
        recalc_result = await recalculate_irr_after_activity_change(portfolio_fund_id, db, activity_date)
        existing_irr_after = await db.fetch('SELECT * FROM portfolio_fund_irr_values WHERE fund_id = $1 AND date >= $2', portfolio_fund_id, activity_date)
        existing_portfolio_irr_after = await db.fetch('SELECT * FROM portfolio_irr_values WHERE portfolio_id = $1 AND date >= $2', portfolio_id, activity_date)
        return {'test_status': 'completed', 'portfolio_fund_id': portfolio_fund_id, 'portfolio_id': portfolio_id, 'activity_date_used': activity_date, 'recalculation_result': recalc_result, 'before_state': {'fund_irr_count': len(existing_irr_before) if existing_irr_before else 0, 'portfolio_irr_count': len(existing_portfolio_irr_before) if existing_portfolio_irr_before else 0, 'fund_irr_values': [dict(row) for row in existing_irr_before] if existing_irr_before else [], 'portfolio_irr_values': [dict(row) for row in existing_portfolio_irr_before] if existing_portfolio_irr_before else []}, 'after_state': {'fund_irr_count': len(existing_irr_after) if existing_irr_after else 0, 'portfolio_irr_count': len(existing_portfolio_irr_after) if existing_portfolio_irr_after else 0, 'fund_irr_values': [dict(row) for row in existing_irr_after] if existing_irr_after else [], 'portfolio_irr_values': [dict(row) for row in existing_portfolio_irr_after] if existing_portfolio_irr_after else []}}
    except HTTPException:
        raise
    except Exception as e:
...
@router.post('/holding-activity-logs/compare-irr-performance')
async def compare_irr_performance(portfolio_fund_id: int=Query(..., description='Portfolio fund ID to test performance with'), activity_date: Optional[str]=Query(None, description='Activity date to use for testing (YYYY-MM-DD format)'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        return {'test_status': 'completed', 'portfolio_fund_id': portfolio_fund_id, 'portfolio_id': portfolio_id, 'activity_date_used': activity_date, 'performance_comparison': {'targeted_method': {'duration_seconds': round(targeted_duration, 2), 'dates_processed': targeted_result.get('dates_processed', 0), 'method': 'handle_activity_changes_batch', 'efficiency': 'high'}, 'historical_method': {'duration_seconds': round(historical_duration, 2), 'dates_processed': historical_result.get('dates_processed', 0), 'method': 'handle_historical_changes', 'efficiency': 'low'}, 'performance_metrics': {'speed_improvement_factor': round(speed_improvement, 1), 'time_saved_seconds': round(time_saved, 2), 'efficiency_gain_percent': round((historical_duration - targeted_duration) / historical_duration * 100, 1) if historical_duration > 0 else 0}}, 'recommendation': {'use_targeted_for': 'Single activity changes, regular operations', 'use_historical_for': 'Major historical corrections, data fixes', 'summary': f'Targeted method is {speed_improvement:.1f}x faster and saves {time_saved:.1f} seconds'}}
    except HTTPException:
        raise
    except Exception as e:
...
@router.post('/holding-activity-logs/recalculate-all-portfolio-irr')
async def recalculate_all_portfolio_irr(portfolio_id: int=Query(..., description='Portfolio ID to recalculate IRR for all funds'), activity_date: Optional[str]=Query(None, description='Activity date to use for recalculation (YYYY-MM-DD format)'), db=Depends(get_db)):
    """
    ...
    """
    try:
        portfolio_funds = await db.fetch('SELECT id, available_funds_id FROM portfolio_funds WHERE portfolio_id = $1', portfolio_id)
        if not portfolio_funds:
        ...
        for pf in portfolio_funds:
            ...
            fund_details = await db.fetchrow('SELECT fund_name, isin_number FROM available_funds WHERE id = $1', available_funds_id)
            fund_name = 'Unknown'
            if fund_details:
                fund_name = fund_details['fund_name']
            try:
                irr_result = await recalculate_irr_after_activity_change(portfolio_fund_id=portfolio_fund_id, db=db, activity_date=activity_date)
                results.append({'portfolio_fund_id': portfolio_fund_id, 'fund_name': fund_name, 'success': irr_result.get('success', False), 'result': irr_result})
            except Exception as e:
                error_msg = str(e)
                logger.error(f'Failed to recalculate IRR for fund {portfolio_fund_id}: {error_msg}')
                results.append({'portfolio_fund_id': portfolio_fund_id, 'fund_name': fund_name, 'success': False, 'error': error_msg})
        successful_count = sum((1 for r in results if r['success']))
        failed_count = len(results) - successful_count
        return {'success': True, 'portfolio_id': portfolio_id, 'total_funds': len(results), 'successful_recalculations': successful_count, 'failed_recalculations': failed_count, 'results': results}
    except Exception as e:
...
@router.post('/holding-activity-logs/bulk', response_model=List[dict])
async def create_bulk_holding_activity_logs(activities: List[HoldingActivityLogCreate], skip_irr_calculation: bool=Query(False, description='Skip IRR recalculation for transaction coordination'), db=Depends(get_db)):
    """
    ...
    try:
        ...
        for log in activities:
            ...
            if isinstance(log.activity_timestamp, date) and (not isinstance(log.activity_timestamp, datetime)):
                ...
                try:
                    ...
                    import re
                    date_match = re.match('(\\d{4}-\\d{2}-\\d{2})', log.activity_timestamp)
                    if date_match:
                ...
                activity_datetime = log.activity_timestamp
            activity_record = {'portfolio_fund_id': log.portfolio_fund_id, 'product_id': log.product_id, 'activity_type': log.activity_type, 'activity_timestamp': activity_datetime, 'amount': float(log.amount) if log.amount is not None else None, 'created_at': datetime.utcnow()}
        ...
        created_activities = await SequenceManager.bulk_insert_with_reserved_ids(db=db, table_name='holding_activity_log', data=activity_data, sequence_name='holding_activity_log_id_seq')
        ...
        if not skip_irr_calculation and created_activities:
            ...
            for portfolio_fund_id in portfolio_funds:
                try:
                    ...
                    irr_result = await recalculate_irr_after_activity_change(portfolio_fund_id, db, activity_date=None)
                    recalc_results.append({'portfolio_fund_id': portfolio_fund_id, 'success': irr_result.get('success', False), 'irr_values_updated': irr_result.get('irr_values_updated', 0)})
                    ...
                    recalc_results.append({'portfolio_fund_id': portfolio_fund_id, 'success': False, 'error': str(irr_error)})
        ...
        response_data = {'activities': created_activities, 'summary': {'total_requested': len(activities), 'total_created': success_count, 'success_rate': success_count / len(activities) * 100 if activities else 0, 'irr_recalculation_skipped': skip_irr_calculation, 'sequence_reservation_used': True}}
