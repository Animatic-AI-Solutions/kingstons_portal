-- ============================================================================
-- Migration 003: SAFE Add fixed_fee_direct column
-- Created: 2025-11-25
-- Risk: LOW (Additive only, preserves existing functionality)
...
-- ============================================================================
-- STEP 1: Add Column to client_products Table (SAFE - Additive only)
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Update company_revenue_analytics View (SAFE - Adds new columns, preserves existing)
-- ============================================================================
-- This preserves ALL existing columns and only adds new ones
...
SELECT
    -- NEW: Add fixed_fee_direct revenue calculation
    sum(
        CASE
    ...
    -- EXISTING: Keep exact same calculation for fixed_fee_facilitated
    sum(
        CASE
    ...
    -- EXISTING: Keep exact same calculation for percentage_fee_facilitated
    sum(
        CASE
    ...
    -- UPDATED: Modify total_annual_revenue to include fixed_fee_direct (this is the only behavior change)
    sum(
        CASE
            WHEN (cp.status = 'active'::text) THEN (
    ...
    -- EXISTING: Keep all other columns exactly the same
    ...
    -- NEW: Add average for fixed_fee_direct
    avg(
        CASE
...
-- ============================================================================
-- STEP 3: Update products_list_view View (SAFE - Only adds fixed_fee_direct column)
-- ============================================================================
...
SELECT
    ...
    cp.fixed_fee_direct,  -- NEW: Add fixed_fee_direct
    cp.fixed_fee_facilitated,  -- EXISTING: Keep exact same
    cp.percentage_fee_facilitated  -- EXISTING: Keep exact same
FROM (((((((client_products cp
...
GROUP BY
    ...
    cp.fixed_fee_direct, cp.fixed_fee_facilitated, cp.percentage_fee_facilitated;  -- UPDATED: Add fixed_fee_direct to GROUP BY

-- ============================================================================
-- STEP 4: Update revenue_analytics_optimized View (SAFE - Only adds fixed_fee_direct column)
-- ============================================================================
...
-- Check 4: Verify existing revenue calculations still work
SELECT 
    'Backward Compatibility Test' as test_type,
    COUNT(*) as active_products,
    -- Test that existing fixed+percentage calculations still work exactly the same
    SUM(
        COALESCE(fixed_fee_facilitated::numeric, 0) +
        CASE 
            WHEN percentage_fee_facilitated IS NOT NULL 
            THEN 0  -- Just test the column access, not actual calculation
            ELSE 0 
        END
    ) as existing_calculations_work
FROM client_products 
WHERE status = 'active';
...
-- Success message
SELECT '=================================================================' as divider;
SELECT '🎉 MIGRATION 003 COMPLETED SAFELY! 🎉' as status;
SELECT '=================================================================' as divider;
SELECT 'Column added: fixed_fee_direct (NULL for existing products)' as change1;
SELECT 'Views updated with backward compatibility maintained' as change2;
SELECT 'New computed column: total_fixed_direct_revenue, avg_fixed_direct_fee' as change3;
SELECT 'Existing applications will continue to work unchanged' as change4;
