-- =========================================================
-- Kingston's Portal - Create Database Views and Functions
-- This script creates all the views and functions needed for analytics
-- =========================================================
...
SELECT 
    ...
    STRING_AGG(DISTINCT cg.name, ', ' ORDER BY cg.name) as client_groups,
    STRING_AGG(DISTINCT cp.product_name, ', ' ORDER BY cp.product_name) as products
...
SELECT 
    ...
    STRING_AGG(DISTINCT name, ', ' ORDER BY name) as client_names
FROM client_groups
...
SELECT 
    ...
    STRING_AGG(DISTINCT po.known_as, ', ' ORDER BY po.known_as) as product_owners,
    STRING_AGG(DISTINCT ap.name, ', ' ORDER BY ap.name) as providers,
...
SELECT 
    ...
    STRING_AGG(af.fund_name, ', ' ORDER BY apf.target_weighting DESC) as fund_names
...
SELECT 
    ...
    STRING_AGG(DISTINCT COALESCE(po.known_as, CONCAT(po.firstname, ' ', po.surname)), ', ' ORDER BY po.firstname) as owners
