# =========================================================
# Import Data to Local PostgreSQL - Kingston's Portal (Fixed)
...
foreach ($table in $tables) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "    Checking first few lines of $fileName..." -ForegroundColor Gray
        $sampleLines = Get-Content $filePath -TotalCount 3
        foreach ($line in $sampleLines) {
            if ($line.Length -gt 100) {
                Write-Host "    $($line.Substring(0, 100))..." -ForegroundColor Gray
            } else {
...
Write-Host "`nStep 4: Verifying data integrity..." -ForegroundColor Cyan
...
foreach ($table in $tables) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($rowCount -gt 0) {
            ...
            Write-Host "  $table : 0 rows (empty)" -ForegroundColor Gray
        }
    }
}
