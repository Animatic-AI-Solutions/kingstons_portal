...
# ====================================================================
# This script compares the expected columns from Supabase export files
# with the actual columns in local PostgreSQL tables
# ====================================================================
...
foreach ($table in $tables) {
    Write-Host "`n🔍 Analyzing table: $table" -ForegroundColor White
...
if ($schemaMismatches.Count -gt 0) {
    Write-Host "`n❌ TABLES REQUIRING FIXES:" -ForegroundColor Red
    
    foreach ($mismatch in $schemaMismatches) {
        Write-Host "`n  🔧 Table: $($mismatch.tableName)" -ForegroundColor Yellow
    ...
    Write-Host "  1. Run the generated fix script: .\migration_scripts\25_fix_schema_mismatches.ps1" -ForegroundColor White
    ...
    Write-Host "`n✅ ALL SCHEMAS MATCH!" -ForegroundColor Green
