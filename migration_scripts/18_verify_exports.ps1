# =========================================================
# Verify SQL Exports - Check exported files
# =========================================================
...
if (!(Test-Path $exportDir)) {
    Write-Host "❌ Export directory '$exportDir' not found!" -ForegroundColor Red
    Write-Host "   Create the directory and export your SQL files there." -ForegroundColor Yellow
    exit 1
}

Write-Host "`n📁 Checking exported files..." -ForegroundColor Cyan
...
foreach ($table in $expectedTables) {
    ...
    if (Test-Path $filePath) {
        ...
        if ($fileSize -eq 0) {
            $emptyFiles += $table
            Write-Host "  WARNING: $fileName (0 bytes - EMPTY)" -ForegroundColor Yellow
        } elseif ($fileSize -lt 100) {
            Write-Host "  WARNING: $fileName ($fileSize bytes - Very small)" -ForegroundColor Yellow
        } else {
            $sizeKB = [math]::Round($fileSize / 1KB, 2)
            Write-Host "  SUCCESS: $fileName ($sizeKB KB)" -ForegroundColor Green
        }
    } else {
        $missingFiles += $table
...
if ($foundFiles.Count -eq $expectedTables.Count -and $emptyFiles.Count -eq 0) {
    ...
    Write-Host "ERROR: Export incomplete - missing $($missingFiles.Count) files" -ForegroundColor Red
    Write-Host "   Continue exporting the missing tables from Supabase Dashboard" -ForegroundColor White
}

Write-Host "`nNext Steps:" -ForegroundColor Cyan
Write-Host "1. Fix any missing or empty files" -ForegroundColor White
