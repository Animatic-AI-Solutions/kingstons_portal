...
CREATE TABLE public.client_products (
    ...
    product_name text NULL, -- Renamed from account_name
    status text NOT NULL DEFAULT 'active'::text,
    start_date date NOT NULL,
    end_date date NULL,
    weighting numeric(5, 2) NULL, -- Purpose of this weighting still potentially ambiguous
    created_at timestamp WITH time zone NOT NULL DEFAULT now(),
    plan_number text NULL,
    provider_id bigint NULL,
    product_type text NULL, -- Added to replace separate available_products table
    CONSTRAINT client_products_pkey PRIMARY KEY (id), -- Renamed constraint
    CONSTRAINT client_products_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients (id), -- Renamed constraint
    CONSTRAINT client_products_provider_id_fkey FOREIGN KEY (provider_id) REFERENCES public.available_providers (id) -- Consistent naming
) TABLESPACE pg_default;
