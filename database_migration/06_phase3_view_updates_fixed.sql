...
-- First, let's identify and handle any dependent views
-- Save the original view definition by creating a backup
DO $$
BEGIN
    -- Check if the view exists before backing it up
    IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'client_group_complete_data') THEN
        -- Create backup by selecting from existing view
        EXECUTE 'CREATE TABLE IF NOT EXISTS client_group_complete_data_backup_phase3 AS SELECT * FROM client_group_complete_data LIMIT 0';
        RAISE NOTICE 'Backup table created for client_group_complete_data';
    END IF;
END $$;

-- Drop the existing view and any dependencies (CASCADE will handle dependencies)
DROP VIEW IF EXISTS public.client_group_complete_data CASCADE;

-- Log the view drop
INSERT INTO advisor_migration_log (migration_step, status, details)
VALUES ('PHASE3_DROP_VIEWS', 'SUCCESS', 'Dropped existing views to handle dependencies');

-- ===================
-- RECREATE UPDATED VIEWS
-- ===================

-- Recreate client_group_complete_data view with advisor relationships
CREATE VIEW public.client_group_complete_data AS
SELECT 
...
-- Log view recreation
INSERT INTO advisor_migration_log (migration_step, status, details)
VALUES ('PHASE3_RECREATE_MAIN_VIEW', 'SUCCESS', 'Recreated client_group_complete_data view with advisor relationships');

-- ===================
-- UPDATE/CREATE SEARCH FUNCTION
-- ===================

-- Drop and recreate the global search function
...
-- Log search function update
INSERT INTO advisor_migration_log (migration_step, status, details)
VALUES ('PHASE3_UPDATE_SEARCH', 'SUCCESS', 'Updated global search function to use advisor relationships');
...
-- Show clients without advisors (limited to 5 for readability)
SELECT * FROM clients_without_advisors LIMIT 5;

-- Quick test of the updated view
SELECT 
    client_group_name,
    advisor_name,
    advisor_email,
...
WHERE advisor_name IS NOT NULL OR legacy_advisor_text IS NOT NULL
LIMIT 10;

-- Log Phase 3 completion
INSERT INTO advisor_migration_log (migration_step, status, details, affected_rows)
VALUES ('PHASE3_COMPLETE', 'SUCCESS', 'Phase 3 database view updates completed successfully (FIXED version)',
        (SELECT COUNT(*) FROM client_groups));

-- Display completion message
SELECT 
    'PHASE 3 COMPLETED (FIXED)' as status,
