...
-- New product_owners table with detailed name structure
...
CREATE TABLE public.template_portfolio_generations (
    ...
    CONSTRAINT uq_available_portfolio_version UNIQUE (available_portfolio_id) -- Ensures each version number is unique per portfolio
) TABLESPACE pg_default;
...
CREATE INDEX IF NOT EXISTS idx_product_owners_firstname ON public.product_owners USING btree (firstname) TABLESPACE pg_default;
CREATE INDEX IF NOT EXISTS idx_product_owners_surname ON public.product_owners USING btree (surname) TABLESPACE pg_default;
CREATE INDEX IF NOT EXISTS idx_product_owners_known_as ON public.product_owners USING btree (known_as) TABLESPACE pg_default;
...
SELECT
    ...
    CASE 
        WHEN po.known_as IS NOT NULL AND po.known_as != '' THEN po.known_as
        WHEN po.firstname IS NOT NULL AND po.surname IS NOT NULL THEN CONCAT(po.firstname, ' ', po.surname)
        WHEN po.firstname IS NOT NULL THEN po.firstname
        ELSE 'Unknown'
    END AS product_owner_name,
    po.firstname AS product_owner_firstname,
    po.surname AS product_owner_surname,
    po.known_as AS product_owner_known_as,
...
GROUP BY 
    tpg.id, tpg.available_portfolio_id, tpg.generation_name, tpg.status;
    ...
    SELECT * FROM (
        ...
        SELECT 
            ...
            CASE 
                ...
                ELSE 'Unknown'
            END as name,
            'Product Owner' as description,
            COALESCE('Status: ' || po.status, '') as additional_info
        FROM product_owners po
        WHERE po.status != 'inactive'
        AND (
            (po.firstname IS NOT NULL AND po.firstname ILIKE '%' || search_query || '%') OR
            (po.surname IS NOT NULL AND po.surname ILIKE '%' || search_query || '%') OR
            (po.known_as IS NOT NULL AND po.known_as ILIKE '%' || search_query || '%')
        )
    ) AS search_results
    ORDER BY 
        CASE 
